...
description: This skill should be used when the user asks to "write a VoxEngine scenario", "review VoxEngine code", "debug a Voximplant call flow", "build an inbound or outbound voice agent", or "validate VoxEngine APIs and events".
---
...
If the user does not have `voximplant-management-api`, ask them to add or install that companion skill in their current environment before continuing with platform-level debugging.
...
If deployment, platform resources, service account JSON, Voximplant secrets, users, routing rules, call history, or logs are needed, read or request `voximplant-management-api`. If the user does not have that companion skill, ask them to add or install it in their current agent environment first.
