...
description: This skill should be used when the user asks to "create or update Voximplant resources", "deploy a VoxEngine scenario", "manage Voximplant applications, users, or rules", "retrieve Voximplant logs or recordings", or "automate the Voximplant Management API".
---
...
If the task also needs VoxEngine code, read or request `voximplant-voxengine-dev`. If the user does not have that companion skill available, ask them to install or add it in their current agent environment before proceeding with code-level work.
...
Switch to or request `voximplant-voxengine-dev` when the task requires scenario authoring, code review, runtime event ordering, or code-level debugging. If the user does not have that skill, ask them to add or install the companion skill in their current environment before continuing.
