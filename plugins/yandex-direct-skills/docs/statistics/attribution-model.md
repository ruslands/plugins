...
- **Первый показ из Директа кросс-девайс** — одним из источников конверсии на сайте является первый показ медийного объявления в любом формате за последние 1, 3, 7, 14, 21 или 30 дней ([выбрается в настройках](../statistics/attribution-model.md#window)). Модель позволяет учитывать в статистике влияние медийных объявлений на визиты и конверсии. Засчитываются переходы со всех используемых устройств. Доступна только в [Мастере отчетов](../statistics/attribution-model.md#brand-statistics).
- **Последний показ из Директа кросс-девайс** — одним из источников конверсии на сайте является последний показ медийного объявления в любом формате за последние 1, 3, 7, 14, 21 или 30 дней ([выбрается в настройках](../statistics/attribution-model.md#window)). Модель позволяет учитывать в статистике влияние медийных объявлений на визиты и конверсии. Засчитываются переходы со всех используемых устройств. Доступна только в [Мастере отчетов](../statistics/attribution-model.md#brand-statistics).
...
При оплате за конверсии воспользуйтесь [советами](../statistics/attribution-model.md#conversions) ниже.
...
В [Мастере отчетов](../statistics/report-wizard.md) можно посмотреть статистику по любой модели атрибуции. Чтобы контролировать соблюдение целевых показателей в конверсионных стратегиях, например [Максимум конверсий](../strategies/average-cpa.md), важно выбирать в отчете ту же модель атрибуции, что указана в настройках стратегии.
