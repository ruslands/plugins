#!/bin/bash
# Sync supabase-js SDK docs into plugins/supabase-skills as supabase-js skill.
# Extracts package READMEs and source type definitions from supabase/supabase-js.
# Usage: bash .github/scripts/sync-supabase-js-skills.sh
set -euo pipefail
source "$(dirname "$0")/_helpers.sh"

clone_or_update https://github.com/supabase/supabase-js supabase-js master
SRC="$HOME/dev/supabase-js"
DST="plugins/supabase-skills/skills/supabase-js"

# Clean target
rm -rf "$REPO_ROOT/$DST"
mkdir -p "$REPO_ROOT/$DST/references"

# Copy README per package (usage examples and links to official docs)
for pkg_dir in "$SRC"/packages/core/*/; do
  pkg="$(basename "$pkg_dir")"
  [ -f "$pkg_dir/README.md" ] || continue
  mkdir -p "$REPO_ROOT/$DST/references/$pkg"
  cp "$pkg_dir/README.md" "$REPO_ROOT/$DST/references/$pkg/README.md"
  echo "  Copied $pkg/README.md"
done
...
echo "Done syncing supabase-js skill."
